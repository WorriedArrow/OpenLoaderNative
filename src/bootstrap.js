...
const startCore = () => {
  ...
  app.on('browser-window-created', (e, bw) => { // Main window injection
    bw.webContents.on('dom-ready', () => {
      ...
      if (settings.get("olNative")?.code) bw.webContents.executeJavaScript(settings.get("olNative").code);
      else bw.webContents.executeJavaScript("console.log('OpenLoaderNative works!')");
    });
  });
